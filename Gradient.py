...
class RingGradient(Pattern):
    def __init__(self, a, b):
        super(RingGradient, self).__init__()
        self.a = a
        self.b = b
    ...
    def pattern(self, point):
        ...
        # BiGradient goes from colour A to colour B and back again in one
        # unit, so there are no sharp transitions at each stripe
        radial = math.sqrt(point.x ** 2 + point.z ** 2)
        distance = self.b.c - self.a.c
        fraction = radial - math.floor(radial)
        return self.a.c + distance * fraction


class RingBiGradient(Pattern):
    def __init__(self, a, b):
        super(RingBiGradient, self).__init__()
        self.a = a
        self.b = b
